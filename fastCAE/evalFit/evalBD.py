...
os.makedirs(f"{fig_dir}/BDBR-fitOnly", exist_ok=True)
os.makedirs(f"{table_dir}/BDBR-fitOnly", exist_ok=True)

cols = ["seqName", "sceneId", "func", "preset", "size", "qp", "pred_target"]
def process_group(args):
    group, seqName, sceneId, preset, distortion = args
    ...
    for func in group[group["target"] == distortion]["func"].unique():
        distortion_df = group[
        ...
        os.makedirs(f"{table_dir}/BDBR-fitOnly/rd-{distortion}/{seqName}-scene{sceneId}", exist_ok=True)
        ...
        convex_df.to_csv(f"{table_dir}/BDBR-fitOnly/rd-{distortion}/{seqName}-scene{sceneId}/{seqName}-scene{sceneId}_{func}_{preset}.csv", index=False)

        """ 2. 画图 """
        fig, axes = plt.subplots(1, 2, figsize=(8, 3.5))
        sns.lineplot(ax=axes[0], data=group_rd, x="pred_bitrate", y=f"pred_{distortion_type}", hue="size", marker="o")
        axes[0].plot(pred_hull_points[:, 0], pred_hull_points[:, 1], linestyle='--', color='purple', marker="x", lw=2, alpha=0.8, label="predicted ladder")
        axes[0].legend()
        ...
        axes[1].plot(actual_hull_points[:, 0], actual_hull_points[:, 1], linestyle='--', color='purple', marker="x", lw=2, alpha=0.8, label="actual ladder")
        axes[1].legend()
        ...
        os.makedirs(f"{fig_dir}/BDBR-fitOnly/rd-{distortion}/{seqName}-scene{sceneId}", exist_ok=True)
        plt.savefig(f"{fig_dir}/BDBR-fitOnly/rd-{distortion}/{seqName}-scene{sceneId}/{seqName}-scene{sceneId}_{func}_{preset}.pdf", format="pdf")
        plt.close()


def DEBUG(pred_df):
    group = pred_df[(pred_df["seqName"] == "Lecture-42c3") & (pred_df["sceneId"] == 0) & (pred_df["preset"] == "faster")].reset_index(drop=True)
    process_group((group, "Lecture-42c3", 0, "faster", "psnr"))   # group, seqName, sceneId, preset, distortion
...
if __name__ == "__main__":
    ...
    fit_df = pd.read_csv(f"{table_dir}/fitCurve/combined_fits.csv")
    fit_df = fit_df.rename(columns={"pred_value": "pred_target"})

    # DEBUG(fit_df)
    grouped = fit_df.groupby(["seqName", "sceneId", "preset"], as_index=False)

    distortions = ["psnr", "log2psnr", "ssim", "log2ssim", "vmaf", "log2vmaf"]
    tasks = []
    for distortion in distortions:
        for (seqName, sceneId, preset), group in tqdm(grouped, desc=f"BD-BR ({distortion})"):
            tasks.append((group, seqName, sceneId, preset, distortion))
