...
def _single_regression(train_df, test_df, preset_x, size_x, regressor_cls, regressor_params, metrics=None, scale=True, verbose=False):
    ...
    for preset in train_df["preset"].unique():
        for size in train_df["size"].unique():
            ...
            if verbose:
                print(
                    f"preset: {preset}, size: {size}, input: ({preset_x})x({size_x}), regressor: {regressor_name}, train_rmse: {train_rmse}, train_r2: {train_r2}, train_evs: {train_evs}, test_rmse: {test_rmse}, test_r2: {test_r2}, test_evs: {test_evs}")

            rows.append(
                [preset, size, f"({preset_x}x{size_x})", regressor_name, train_rmse, train_r2, train_evs, test_rmse,
                 test_r2, test_evs])
        ...
        for func in ["linear", "power", "quadratic2"]:
            for size_x in ["360P", "540P", "720P", "1080P"]:
                for preset_x in ["faster", "medium", "slower"]:
                    process_task((target, func, preset_x, size_x))
...
if __name__ == '__main__':
    ...
    all_rlts_df.to_csv(f"{rlt_dir}/all_rlts_dual.csv", index=False)
    all_preds_df.to_csv(f"{rlt_dir}/all_preds_dual.csv", index=False)